        ...
        <div class="p-10 max-w-[1400px] mx-auto w-full">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                <div class="flex items-center gap-6">
                    <div class="w-20 h-20 bg-white rounded-3xl shadow-sm border border-slate-100 flex items-center justify-center overflow-hidden p-2">
                        <span class="material-symbols-outlined text-4xl text-cyan-600">monitoring</span>
                    </div>
                    <div>
                        ...
                        <p class="text-slate-500 mt-1 font-medium flex items-center gap-2">
                            Pantau performa event, pendaftaran peserta, dan pendapatan Anda.
                        </p>
                    </div>
                </div>
                <div class="flex gap-3">
                    <a href="{{ url('/organizer/peserta') }}" class="bg-white text-slate-600 px-6 py-3 rounded-2xl font-bold text-xs border border-slate-200 hover:bg-slate-50 transition-all flex items-center gap-2">
                        <span class="material-symbols-outlined text-sm">groups</span> Kelola Peserta
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:border-cyan-200 transition-all">
                    ...
                    <div class="text-4xl font-black text-slate-900">{{ $stats['active_events'] ?? 0 }}</div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Event Aktif</p>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:border-emerald-200 transition-all">
                    ...
                    <div class="text-4xl font-black text-slate-900">{{ $stats['total_participants'] ?? 0 }}</div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Total Pendaftar Keseluruhan</p>
                </div>
                ...
                <div class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm group hover:border-amber-200 transition-all">
                    ...
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Total Pendapatan Tiket</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-12 gap-8">
                <div class="col-span-12 lg:col-span-8 space-y-8">
                    ...
                    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm h-80 relative flex flex-col justify-between">
                        <div>
                            <h3 class="text-lg font-black text-slate-900 tracking-tight">Progres Pendaftaran Harian</h3>
                            <p class="text-xs text-slate-400 font-medium uppercase tracking-widest mt-1">Statistik Simulasi Platform</p>
                        </div>
                    ...
                    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                            ...
                            <a href="{{ url('/organizer/peserta') }}" class="text-[10px] font-black text-cyan-600 uppercase tracking-widest hover:underline">Lihat Semua</a>
                        </div>
                        <div class="divide-y divide-slate-50">
                            @forelse($stats['recent_registrations'] ?? [] as $reg)
                                @php $initials = strtoupper(substr($reg['participant_name'] ?? 'U', 0, 2)); @endphp
                                <div class="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-400 text-xs">
                                            {{ $initials }}
                                        </div>
                                        <div>
                                            <h4 class="font-bold text-slate-900 text-sm">{{ $reg['participant_name'] ?? 'Unknown' }}</h4>
                                            <p class="text-[10px] text-slate-400 font-medium">{{ $reg['email'] ?? '-' }}</p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        @if(($reg['status'] ?? '') == 'confirmed' || strtolower($reg['status'] ?? '') == 'lunas')
                                            <span class="px-2 py-0.5 bg-emerald-50 text-emerald-600 text-[9px] font-black rounded uppercase border border-emerald-100">LUNAS</span>
                                        @else
                                            <span class="px-2 py-0.5 bg-amber-50 text-amber-600 text-[9px] font-black rounded uppercase border border-amber-100">PENDING</span>
                                        @endif
                                        <p class="text-[9px] text-slate-400 font-bold mt-1.5 uppercase tracking-widest">{{ $reg['event_title'] ?? '-' }}</p>
                                    </div>
                                </div>
                            @empty
                ...
                <div class="col-span-12 lg:col-span-4 space-y-8">
                    <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
                        <div class="relative z-10">
                            ...
                            <p class="text-xs text-slate-400 font-medium leading-relaxed mb-8">Siap menyambut peserta? Buka terminal scanner untuk memvalidasi tiket QR peserta event Anda.</p>
                            <a href="{{ url('/organizer/checkin') }}" class="inline-flex items-center gap-2 bg-white text-slate-900 px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl shadow-white/5">
                    ...
                    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                        <h3 class="text-sm font-black text-slate-900 mb-6 uppercase tracking-widest flex items-center gap-2">
                            <span class="w-1.5 h-4 bg-cyan-500 rounded-full"></span> Modul Acara
                        </h3>
                        <div class="grid grid-cols-1 gap-3">
                            <a href="{{ url('/organizer/seating') }}" class="p-4 bg-slate-50 rounded-2xl flex items-center justify-between group hover:bg-cyan-50 transition-all border border-transparent hover:border-cyan-100">
                                <div class="flex items-center gap-4">
                            ...
                            <a href="{{ url('/organizer/lucky_draw') }}" class="p-4 bg-slate-50 rounded-2xl flex items-center justify-between group hover:bg-amber-50 transition-all border border-transparent hover:border-amber-100">
                                <div class="flex items-center gap-4">
                            ...
                            <a href="{{ url('/organizer/sertifikat') }}" class="p-4 bg-slate-50 rounded-2xl flex items-center justify-between group hover:bg-emerald-50 transition-all border border-transparent hover:border-emerald-100">
                                <div class="flex items-center gap-4">
                                    <span class="material-symbols-outlined text-slate-400 group-hover:text-emerald-600">workspace_premium</span>
