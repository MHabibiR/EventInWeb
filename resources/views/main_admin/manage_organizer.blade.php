            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                ...
                <div class="flex gap-4">
                    <div class="bg-white px-6 py-4 rounded-3xl border border-slate-100 shadow-sm text-center">
                        ...
                        <p class="text-2xl font-black text-amber-500">{{ $stats['pending'] ?? 0 }} <span class="text-xs text-slate-300 font-bold">Akun</span></p>
                    </div>
                    <div class="bg-white px-6 py-4 rounded-3xl border border-slate-100 shadow-sm text-center">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Organizer</p>
                        <p class="text-2xl font-black text-slate-900">{{ $stats['total'] ?? 0 }} <span class="text-xs text-slate-300 font-bold">Akun</span></p>
                    </div>
                </div>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-sm divide-y divide-slate-100">
                            @forelse($organizers as $org)
                                @php $status = $org['status'] ?? ''; @endphp
                                <tr class="hover:bg-slate-50/50 transition-colors {{ $status == 'pending' ? 'bg-amber-50/10' : '' }}">
                                    <td class="p-6 pl-10">
                                        <div class="flex items-center gap-4">
                                            <div class="w-12 h-12 rounded-2xl {{ $status == 'pending' ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400' }} flex items-center justify-center">
                                                <span class="material-symbols-outlined">{{ ($org['type'] ?? '') == 'Individu' ? 'person_pin' : 'corporate_fare' }}</span>
                                            </div>
                                            <div>
                                                <div class="font-black text-slate-900 leading-tight">{{ $org['name'] ?? 'Instansi Tidak Diketahui' }}</div>
                                                <div class="text-[10px] {{ $status == 'pending' ? 'text-amber-600' : 'text-cyan-600' }} font-black uppercase tracking-widest mt-1">
                                                    {{ $org['type'] ?? 'Organizer' }}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <td class="p-6">
                                        <div class="text-lg font-black {{ $status == 'pending' ? 'text-slate-300' : 'text-slate-900' }}">{{ $org['total_events'] ?? 0 }}</div>
                                        <div class="text-[9px] text-slate-400 font-bold uppercase">{{ $status == 'pending' ? 'Baru Bergabung' : 'Event Aktif' }}</div>
                                    </td>
                                    <td class="p-6">
                                        @if($status == 'verified')
                                    ...
                                    <td class="p-6 pr-10 text-right">
                                        ...
                                        @if($status == 'pending')
                                            <div class="flex justify-end gap-2">
                                                <form action="{{ url('/admin/manage_organizer/' . ($org['id'] ?? 0) . '/status') }}" method="POST" class="m-0">
                                                    @csrf
                                                    <input type="hidden" name="action" value="verified">
                                                    <button type="submit" class="px-4 py-2 bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase shadow-md shadow-emerald-500/20 hover:scale-105 transition-all cursor-pointer">Verifikasi Akun</button>
                                                </form>
                                                
                                                <form action="{{ url('/admin/manage_organizer/' . ($org['id'] ?? 0) . '/status') }}" method="POST" class="m-0">
                                                    @csrf
                                                    <input type="hidden" name="action" value="rejected">
                                                    <button type="submit" class="px-4 py-2 bg-slate-100 text-slate-400 rounded-xl text-[10px] font-black uppercase hover:bg-slate-200 cursor-pointer">Tolak</button>
                                                </form>
                                            </div>
                                        @else
                                            <form action="{{ url('/admin/manage_organizer/' . ($org['id'] ?? 0) . '/status') }}" method="POST" class="m-0 flex justify-end">
                                                @csrf
                                                <input type="hidden" name="action" value="blocked">
                                                <button type="submit" class="p-2 hover:bg-rose-50 rounded-lg transition-colors text-slate-300 hover:text-rose-500 group cursor-pointer border border-transparent hover:border-rose-100" title="Blokir Akses">
                                ...
                                <tr>
                                    <td colspan="5" class="p-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-slate-500 font-bold text-sm">Belum ada data organizer yang terdaftar.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
