            ...
            <form action="{{ url('/admin/events/store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-12 gap-12">
                    <div class="col-span-12 lg:col-span-8 space-y-10">
                        <section class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm hover:border-cyan-100 transition-colors">
                        ...
                        <section class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm hover:border-cyan-100 transition-colors">
                            ...
                            <div class="grid grid-cols-2 gap-6 mb-6">
                                <div>
                                    ...
                                    <input type="date" name="date_start" id="date-start" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-700">
                                </div>
                                <div>
                                    <label for="date-end" class="block text-sm font-bold text-slate-900 mb-2">Berakhir pada</label>
                                    <input type="date" name="date_end" id="date-end" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-700">
                                </div>
                            </div>
                            <div>
                            ...
                            <div class="mt-4">
                                ...
                                <input type="text" name="venue_address" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-medium text-slate-900" placeholder="Jalan, Kota, Provinsi, Kode Pos">
                            </div>
                        </section>
                        ...
                        <section class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm hover:border-cyan-100 transition-colors">
                            <div class="flex justify-between items-center mb-8">
                                ...
                                <button type="button" onclick="addCategory()" class="text-xs font-black text-cyan-600 bg-cyan-50 px-4 py-2 rounded-xl hover:bg-cyan-100 transition-colors uppercase tracking-widest cursor-pointer">
                                    + Tambah Kategori
                                </button>
                            </div>
                            ...
                            <div id="category-list" class="space-y-4">
                                <div class="grid grid-cols-12 gap-4 p-6 bg-slate-50 rounded-2xl border border-slate-100 relative group">
                                    ...
                                    <div class="col-span-2 flex items-end">
                                        <button type="button" class="w-full h-[46px] flex items-center justify-center text-rose-400 hover:text-rose-600 transition-colors cursor-pointer">
                    ...
                    <div class="col-span-12 lg:col-span-4 space-y-8">
                        <section class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                            ...
                            <label for="cover-upload" class="border-2 border-dashed border-slate-200 rounded-xl h-48 flex flex-col items-center justify-center text-slate-400 hover:bg-slate-50 hover:border-cyan-200 cursor-pointer transition-all group mb-6 block w-full">
                                <span class="material-symbols-outlined text-4xl mb-2 group-hover:scale-110 transition-transform text-slate-300 group-hover:text-cyan-500">add_photo_alternate</span>
                                <span class="text-xs font-bold uppercase tracking-widest group-hover:text-cyan-600">Klik untuk unggah</span>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label for="base-price" class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Harga Dasar (Rp)</label>
                        ...
                        <div class="flex flex-col gap-3 sticky top-28">
                            <button type="submit" name="action" value="draft" class="w-full py-4 rounded-xl font-bold bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all text-xs uppercase tracking-widest cursor-pointer border border-slate-200">
                                Simpan Draf
                            </button>
                            <button type="submit" name="action" value="publish" class="w-full py-4 rounded-xl font-bold text-white primary-gradient shadow-xl shadow-cyan-500/30 active:scale-95 transition-all text-xs uppercase tracking-widest cursor-pointer">
                                Terbitkan Event
                            </button>
                        </div>
                    </div>
    ...
    <script>
        function addCategory() {
            ...
            newItem.innerHTML = `
                ...
                <div class="col-span-2 flex items-end">
                    <button type="button" onclick="this.parentElement.parentElement.remove()" class="w-full h-[46px] flex items-center justify-center text-rose-400 hover:text-rose-600 transition-colors cursor-pointer">
                        <span class="material-symbols-outlined">delete_sweep</span>
                    </button>
                </div>
            `;
